...
def main():
    ...
    if not st.session_state.report_generated:
        ...
        if st.button("Generate Report"):
            if not api_key and not os.getenv("OPENAI_API_KEY"):
                ...
                with st.spinner("Generating report..."):
                    stdout, stderr = run_backend(uploaded_file, selected_languages, cultural_assessment, model, api_key)
        ...
        if report_folder:
            ...
            #         st.markdown(get_binary_file_downloader_html(file, os.path.basename(file)), unsafe_allow_html=True)
